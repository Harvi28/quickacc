...
protected function populateState($ordering = 'lft', $direction = 'asc')
	...
	protected function getListQuery()
	{
		...
		$query->select('a.id as id, a.greeting as greeting, a.published as published, a.created as created, 
				  ...
				  ->from($db->quoteName('jsvcz_helloworld', 'a'));

		// Join over the categories.
		$query->select($db->quoteName('c.title', 'category_title'))
			->join('LEFT', $db->quoteName('jsvcz_categories', 'c') . ' ON c.id = a.catid');
        
		// Join with users table to get the username of the author
		$query->select($db->quoteName('u.username', 'author'))
			->join('LEFT', $db->quoteName('jsvcz_users', 'u') . ' ON u.id = a.created_by');
		...
		$query->select($db->quoteName('u2.username', 'editor'))
			->join('LEFT', $db->quoteName('jsvcz_users', 'u2') . ' ON u2.id = a.checked_out');
            

      $query->select($db->quoteName('l.title', 'language_title') . "," .$db->quoteName('l.image', 'language_image'))
			->join('LEFT', $db->quoteName('jsvcz_languages', 'l') . ' ON l.lang_code = a.language');

		// Join over the associations - we just want to know if there are any, at this stage
		if (JLanguageAssociations::isEnabled())
		{
			$query->select('COUNT(asso2.id)>1 as association')
				->join('LEFT', 'jsvcz_associations AS asso ON asso.id = a.id AND asso.context=' . $db->quote('com_helloworld.item'))
				->join('LEFT', 'jsvcz_associations AS asso2 ON asso2.key = asso.key')
				->group('a.id');
		}
		// Filter: like / search
